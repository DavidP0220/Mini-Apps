...
def create_image(
    ...
    with page.expect_response(lambda r: ASSET_URL_RE.match(r.url) and r.url.endswith(".jpg"), timeout=240000) as resp_info:
...
def create_lipsync_video(
    ...
    lipsync_checkbox = page.get_by_text("Lipsync HD Video")
    lipsync_checkbox.click()
...
def create_silent_video(
    ...
    with page.expect_response(lambda r: ASSET_URL_RE.match(r.url) and r.url.endswith(".jpg"), timeout=240000):
        page.get_by_role("button", name="Create Image").click()
    ...
    video_only_checkbox = page.get_by_text("Video Only (No Sound)")
    video_only_checkbox.click()

    page.get_by_role("button", name="Create Video").click()
    _dismiss_aspect_ratio_confirm(page)
    close_modal(page)

    url = _poll_for_latest_video(page, max_wait=900)
    filename = filename or url.rsplit("/", 1)[-1]
    local_path = _download(url, OUTPUT_DIR / filename)
...
def _poll_for_latest_video(page: Page, max_wait: int = 300, interval: int = 15) -> str:
    ...
    while _time.time() < deadline:
        ...
        # "My AI Videos" resuelve a 2 elementos (nav + título de panel).
        # El título de panel es el que confirma que la carpeta está abierta.
        my_ai_videos = page.locator(".library-panel-category-title", has_text="My AI Videos")
        # Mismo bug que _open_create_with_ai_panel (corregido 2026-08-22):
        # si el panel Media Library ya estaba abierto de un paso anterior en
        # esta misma sesión, "Media Library" deja de resolver como click
        # útil y el bucle se queda esperando en vano. Comprobar primero si
        # ya está visible antes de intentar clickear nada.
        try:
            my_ai_videos.wait_for(state="visible", timeout=2000)
        except PlaywrightTimeoutError:
            for _ in range(3):
                try:
                    media_link.click(timeout=5000)
                except PlaywrightTimeoutError:
                    continue
                try:
                    my_ai_videos.wait_for(state="visible", timeout=4000)
                    break
                except PlaywrightTimeoutError:
                    continue
        # Los 3 reintentos pueden agotarse sin dejar el panel visible (p.ej.
        # el sitio tardó más de lo esperado en cargar). Si eso pasa, NO
        # intentar el click a ciegas (se come un timeout de 60s) — saltar
        # a la siguiente vuelta del while con un respiro corto.
        try:
            my_ai_videos.wait_for(state="visible", timeout=8000)
        except PlaywrightTimeoutError:
            page.wait_for_timeout(interval * 1000)
            continue
        with page.expect_response(lambda r: "get_media" in r.url, timeout=15000) as resp_info:
            my_ai_videos.click()
        data = resp_info.value.json()
        results = data.get("results", [])
        if results:
            newest = results[0]
            print(f"[poll] status={newest.get('status')!r} mediaPath={newest.get('mediaPath')!r} id={newest.get('id')!r}")
            if newest.get("status") == "completed" and newest.get("mediaPath"):
                close_modal(page)
                return newest["mediaPath"]
        else:
            print("[poll] get_media devolvio 0 results")
        close_modal(page)
        page.wait_for_timeout(interval * 1000)
    raise VideoExpressError(f"El video no terminó de renderizar en {max_wait}s")


def import_local_audio(page: Page, audio_path: Path) -> None:
    """Sube un archivo de audio local (ej. voz en off de ElevenLabs) a
    VideoExpress. IMPORTANTE: 'Import Media' en el panel 'Create with AI' es
    para servicios externos (Text to Speech, CloneVoice.ai, Artistly) — NO
    tiene un input de archivo local. El upload real está en el picker de
    Media Library ('Select Image'/'Select Audio'), botón '↑ Upload file'.
    Confirmado 2026-08-20 vía captura de pantalla tras el primer intento
    fallar tan silenciosamente (0 <input type=file> encontrados)."""
    page.get_by_role("link", name="Media Library").click()
    page.wait_for_timeout(1000)
    # OJO: "Audio" matchea DOS elementos — el tab superior (Stock) y la
    # carpeta real dentro de "My Media" — .last es la carpeta. Usar .first
    # aterriza en la pestaña de sonidos de stock (Ambient/Effects/Voiceover),
    # que no tiene upload de archivo propio.
    page.get_by_text("Audio", exact=True).last.click()
    ...
    # que es el que de verdad dispara el file chooser nativo (confirmado
    # 2026-08-20 vía captura de pantalla).
    ...
    # "filechooser") — "+ Add files" solo revela un <input type=file> oculto
    # en el DOM. set_input_files directo funciona sin necesitar
    # expect_file_chooser (confirmado 2026-08-20).
    ...
    close_modal(page)
    # Espera a que la subida termine (aparece en la carpeta)
    page.wait_for_timeout(4000)


def import_local_image(page: Page, image_path: Path) -> None:
    """Sube una imagen local (ej. REFERENCIA_personaje.png o una de sus 4
    vistas front/back/left/right) a la carpeta 'Images' de Media Library,
    para poder usarla luego como Reference Photo de 'Consistent Character'
    (ver mark_consistent_character) en vez de depender solo del picker de
    'My AI Images' con imágenes ya generadas en la app.

    Mismo patrón que import_local_audio(), solo cambia la carpeta destino:
    'Audio' -> 'Images'. Igual que con Audio, el nombre puede matchear tanto
    una pestaña de stock como la carpeta real de 'My Media' — se usa .last
    por el mismo motivo (confirmado en import_local_audio 2026-08-20)."""
    page.get_by_role("link", name="Media Library").click()
    page.wait_for_timeout(1000)
    page.get_by_text("Images", exact=True).last.click()
    page.get_by_role("button", name="Upload file").click(timeout=10000)
    page.get_by_role("button", name="Add files").click(timeout=10000)
